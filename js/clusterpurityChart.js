...
clusterchart = function(){
				...
				selection.each(function(data){
					...
					data.forEach(function(d){
						d.values.forEach(function(e){
							e.total = d.total[0];
							e.color = colorScale(e.count_by);
							if(!colorMap.hasOwnProperty(e.count_by))
								colorMap[e.count_by] = {'key':e.count_by,'color':e.color};
						})
					});
	...
	chart.renderPieChart = function(data){
			...
			var newData = data.values
							.filter(function(d){
								if(legendFilter.indexOf(d.count_by) === -1)
									return true;
								else
									return false;
							});
